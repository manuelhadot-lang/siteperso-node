...
export const SPICE_DIODE_MODEL_LINE = ".model DDEFAULT D(Is=2.52n Rs=0.568 N=1.906 Cjo=1p M=0.03 Eg=1.11 Bv=100 Ibv=0.1u)";
...
export function normalizeValueText(raw) {
    return String(raw || "").trim().replace(",", ".").replace(/µ/g, "u").replace(/Ω/gi, "").replace(/\s+/g, "");
}
...
export function parseSpiceNumericValue(raw, fallback = null) {
    ...
    const multipliers = { t: 1e12, g: 1e9, meg: 1e6, k: 1e3, m: 1e-3, u: 1e-6, n: 1e-9, p: 1e-12, f: 1e-15 };
    if (!unit) {
        return `${base}`;
    }
