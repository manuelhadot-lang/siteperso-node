...
export function buildNgspiceDeckV2(state, options = {}) {
    ...
    if (diagnostics.sourceConnectedToGround === false) {
        warnings.push("Circuit ouvert probable : aucune source ne semble reliee a la masse via un chemin conducteur.");
    }
    if (diagnostics.floatingNets.length > 0) {
        warnings.push(
            `Noeuds possiblement flottants: ${diagnostics.floatingNets.slice(0, 6).join(", ")}${diagnostics.floatingNets.length > 6 ? ", ..." : ""}.`
        );
    }
