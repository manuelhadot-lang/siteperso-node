...
export const ARDUINO_LIB_DOCS = [
    ...
    {
        id: "adafruit_tsl2591",
        match: /TSL2591|Adafruit_TSL/i,
        title: "Adafruit TSL2591 (luminosité I²C)",
        header: "#include <Wire.h>\n#include <Adafruit_TSL2591.h>",
        ctor: "Adafruit_TSL2591 tsl = Adafruit_TSL2591(2591);",
        note: "Grove TSL2591 : SDA→A4 (UNO) ou GPIO10 (ESP32-C3), SCL→A5 / GPIO8, VCC/GND. Adresse I²C 0x29. Double-clic sur le capteur pour régler les lux simulés.",
        commands: [
            { sig: "tsl.begin()", desc: "Initialise le capteur sur le bus I²C.", simSupported: true },
            { sig: "tsl.getFullLuminosity()", desc: "Comptages FULL (16 bits hauts) et IR (16 bits bas).", simSupported: true },
            { sig: "tsl.calculateLux(full, ir)", desc: "Luminosité en lux à partir des comptages.", simSupported: true },
            { sig: "tsl.setGain / setTiming", desc: "Réglages matériels (non simulés).", simSupported: false },
            { sig: "tsl.getEvent()", desc: "Interruptions ALS (non simulées).", simSupported: false },
        ],
    },
    {
        id: "adafruit_bmp280",
    ...
    {
        id: "adafruit_st7735",
        match: /st7735|adafruit.*gfx/i,
        title: "Adafruit ST7735 (Joy-it TFT 1.8″)",
        header: "#include <Adafruit_ST7735.h>\n#include <Adafruit_GFX.h>",
        ctor: "#define TFT_CS  10\n#define TFT_DC   8\n#define TFT_RST  9\nAdafruit_ST7735 tft = Adafruit_ST7735(TFT_CS, TFT_DC, TFT_RST);",
        note: "Joy-it RB-TFT1.8 : SCL→D13, SDA→D11, CS→D10, DC→D8, RES→D9, VCC/GND (logique 3,3 V).",
        commands: [
            { sig: "tft.initR(INITR_BLACKTAB)", desc: "Initialise l’écran ST7735R (128×160).", simSupported: true },
            { sig: "tft.setRotation(n)", desc: "Orientation 0–3 (0=portrait, 1=paysage 90°, 2=180°, 3=270°).", simSupported: true },
            { sig: "tft.fillScreen(color)", desc: "Remplit l’écran (ST7735_BLACK, WHITE, RED…).", simSupported: true },
            { sig: "tft.setTextColor(fg)", desc: "Couleur du texte.", simSupported: true },
            { sig: "tft.setTextSize(n)", desc: "Taille GFX : police 6×8 px × n (n = 1…5).", simSupported: true },
            { sig: "tft.setCursor(x, y)", desc: "Position du curseur en pixels.", simSupported: true },
            { sig: 'tft.print("texte")', desc: "Affiche une chaîne ou un nombre.", simSupported: true },
            { sig: 'tft.println("texte")', desc: "Affiche avec retour à la ligne.", simSupported: true },
            { sig: "delay(ms)", desc: "Pause en millisecondes (animation en simulation).", simSupported: true },
            { sig: "tft.drawPixel / drawLine / drawRect", desc: "Primitives graphiques.", simSupported: false },
        ],
    },
    {
        id: "serial_uart",
    ...
    },
    {
        ...
        commands: [
            ...
            { sig: "analogRead(pin)", desc: "Lit une entrée analogique (UNO : 0–1023 ; ESP32 : 0–4095, 12 bits). analogReadResolution(10) pour 0–1023.", simSupported: true },
            { sig: "analogReadResolution(bits)", desc: "ESP32 : largeur analogRead (10 → 0–1023, 12 → 0–4095).", simSupported: true },
